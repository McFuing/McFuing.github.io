...
由量子力学我们可以知道，当我们知道一个系统的哈密顿量$H$%
,那么我们可以求出它的本征值与本征函数，以及其它的物理量。但是求解哈密顿量$H$%
的本征函数是困难的，那我们试图找到一个包含$H$%
所包含的信息等价的算符，即格林算符\begin{equation}
...
_{n}\right\vert }{z-E_{n}}\label{12}%
\end{align}
...
可以看出$G\left(  z\right)  $%
中包含$H$%
的本征值与本征函数的全部信息,同时当得到$H$%
的全部本征值与本征函数，$G\left(
z\right)  $%
也就已知\footnote{这不就矛盾了吗？想要得到系统的全部信息又要付出较少，不太可能。}%
。
\end{notation}

\subsection{复杂系统的格林函数处理}%


对于不能严格求解的系统，我们将$H$%
分成两个部分\begin{equation}
H=H_{0}+H_{1} \label{4}%
\end{equation}
...
反复迭代\footnote{从$G_{0}$与$H_{1}$的对易关系也可以看出}%
\begin{align}
...
G=G_{0}+G_{0}TG_{0}%
\end{equation}
与式(\ref{2})(\ref{3})比较可得\begin{align}
TG_{0} &  =H_{1}G\\
G_{0}T &  =GH_{1}%
\end{align}
其中\begin{align}
...
\end{align}
对于式(\ref{4}),可以得到$H_{0}%
,H$的本征方程\begin{equation}
H_{0}\left\vert \varphi_{n}\right\rangle =E_{n}\left\vert \varphi
_{n}\right\rangle \label{5}%
\end{equation}%
\begin{equation}
H\left\vert \psi\right\rangle =E\left\vert \psi\right\rangle \label{6}%
\end{equation}
我们可以从$H_{0}$%
的本征函数$\left\vert \varphi_{n}\right\rangle
$以及$G_{0}$求得$H$%
的本征函数$\left\vert \psi\right\rangle
$，所以\begin{equation}
\left(  E_{n}-H_{0}\right)  \left\vert \varphi_{n}\right\rangle =G_{0}%
^{-1}\left(  E_{n}\right)  \left\vert \varphi_{n}\right\rangle =0\label{7}%
\end{equation}%
\begin{equation}
...
\label{8}%
\end{equation}
当$E\neq E_{n}$时，由式(\ref{8})可得\begin{equation}
\left\vert \psi\right\rangle =G_{0}\left(  E\right)  H_{1}\left\vert
\psi\right\rangle
\end{equation}
当$E=E_{n}$时，由式(\ref{7})(\ref{8})%
\[
G_{0}^{-1}\left(  E_{n}\right)  \left\vert \psi\right\rangle -G_{0}%
^{-1}\left(  E_{n}\right)  \left\vert \varphi_{n}\right\rangle =H_{1}%
\left\vert \psi\right\rangle
\]%
\begin{equation}
\left\vert \psi\right\rangle =\left\vert \varphi_{n}\right\rangle
+G_{0}\left(  E_{n}\right)  H_{1}\left\vert \psi\right\rangle \label{20}%
\end{equation}
迭代可得\begin{align}
\left\vert \psi\right\rangle  &  =\left\vert \varphi_{n}\right\rangle
...
E_{n}\right)  \left\vert \varphi_{n}\right\rangle \label{21}%
\end{align}
即有\begin{equation}
...
\subsection{格林函数相关的物理量}%


由于格林函数与哈密顿量一样，含有系统的所有信息，因此凡是通过哈密顿量能够计算出来的物理量，从格林函数也可以求出。

比如我们计算态密度\begin{equation}
\rho\left(  E\right)  =\sum_{n}\delta\left(  E-E_{n}\right)
\footnote{求和是对态求和，相当于求$E=E_{n}%
$%
的态有多少个，即简并度}%
\label{16}%
\end{equation}
整个能量的积分\begin{equation}
...
\subsubsection{格林函数表达}

对式(\ref{1}%
)取坐标表象的矩阵元\begin{equation}
...
\end{equation}
为格林算符在坐标表象下的矩阵元，同理对于哈密顿量\begin{equation}
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjcyNTY2NjQyXX0=
-->