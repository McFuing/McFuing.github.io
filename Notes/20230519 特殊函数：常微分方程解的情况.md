...
$$
p(z)=\sum_{r=1}^{n}\frac{A_rh_r(z)}{z-z_r}+\varphi(z)\tag{16}
$$
其中$A_r$为$p(z)$在$z=z_r$处的留数[^3] ，$h_r(z),\varphi(z)$是全平面解析的函数，因为$z\rightarrow \infty(t\rightarrow0)$是方程的正则奇点，所以由式(15)可知，$\varphi(z\rightarrow\infty)=0$，由刘维尔定理可知[^4] ，$\varphi(z)=0$，所以
$$
p(z)=\sum_{r=1}^{n}\frac{A_rh_r(z)}{z-z_r}\tag{17}
$$
...
同理对于$q(z)$，奇点最多为二阶奇点，
$$
q(z)=\sum_{r=1}^n\left[\frac{B_rg_r(z)}{(z-z_r)^2}+\frac{C_rs_r(z)}{z-z_r}\right]+\varphi(z)\tag{18}
$$
根据式(15)可得,
$$
\frac{1}{t^2}q\left(\frac{1}{t}\right)=\sum_{r=1}^n\frac{B_rg_r\left(\frac{1}{t}\right)}{(1-z_rt)^2}+\sum_{r=1}^n\frac{C_rs_r\left(\frac{1}{t}\right)}{t(1-z_rt)}+\frac{\varphi}{t^2}\tag{19}
$$
所以$\varphi(z\rightarrow\infty)=0$，
$$
\sum_{r=1}^nC_rs_r(z)=0\tag{20}
$$
注意此时我们要求了无穷远点是正则奇点。所以[^5]
$$
q(z)=\sum_{r=1}^n\left[\frac{B_rg_r(z)}{(z-z_r)^2}+\frac{C_rs_r(z)}{z-z_r}\right]\tag{21}
$$
...
方程的奇点为$0,1,a,\infty$
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTcwOTE1Mzk4NywtMTg0NDYzMzIyNCwtMT
czNTQwODUxMSwxNDg2ODE1NzM0LC0xNzEwNjY5MzYxLDIwMzA5
NjE4NjgsNDI5NDgwNDU5LDM3NjUzMDgwNCwxODk5Mjc2MjkyLC
02MjA2Nzg2OTQsLTYwNzEyODgxOSwtNDg4MDQ4MDcsLTc4NTEz
MzY5MCwtMTUxNDM4NDI4MywxMDUzMjM3NTg3LDE5MTU3NTk4MS
wxNjYzMTIyNDcyLC0zNDA3MTMwMThdfQ==
-->