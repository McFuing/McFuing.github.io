...
$$
其中$h(z)$为全纯函数[^8] 。我们将$h(z)$在任意的一个点$z_1$的邻域$|z-z_1|<R$展开，
[^8]:由于函数$f(z)$仅有一个$n$阶极点，这里的全纯函数可以解析延拓到整个复平面。
...
$$
\begin{aligned}
f(z)&=\frac{1}{z-z_0}\sum_{n=0}^{+\infty}h_n(z-z_1)^n\\
&=\frac{1}{z-z_0}\left[h_0+h_1(z-z_1)+h_2(z-z_1)^2+h_3(z-z_1)^3+\dots\right]\\
&=\frac{h_0}{z-z_0}+h_1\frac{z-z_0+z_0-z_1}{z-z_0}+h_2\frac{(z-z_0+z_0-z_1)^2}{z-z_0}+h_3\frac{(z-z_0+z_0-z_1)^3}{z-z_0}+\dots\\
&=\frac{h_0}{z-z_0}+h_1\left(1+\frac{z_0-z_1}{z-z_0}\right)+h_2\left[\frac{(z_0-z_1)^2}{z-z_0}+C_2^1(z_0-z_1)+(z-z_0)\right]\\&+h_3\left[\frac{(z_0-z_1)^3}{z-z_0}+C_3^1(z_0-z_1)^2+C_3^2(z_0-z_1)(z-z_0)+(z-z_0)^2\right]+\dots\\
&=\frac{\sum_{n=0}^{+\infty}h_n(z_0-z_1)^n}{z-z_0}+\sum_{n=1}^{+\infty}C_n^1h_n(z_0-z_1)^{n-1}+(z-z_0)\sum_{n=2}^{+\infty}C_n^2h_n(z_0-z_1)^{n-2}+\dots\\
&=\frac{H_0}{z-z_0}+\sum_{k=0}^{+\infty}H_k(z-z_0)^k\tag{18}
\end{aligned}
$$
其中$H_k=\sum_{n=k+1}^{+\infty}C_n^{k+1}h_n(z_0-z_1)^{n-k-1}$，等式的第二项可以看作$z_1$邻域上的全纯函数[^7]$\phi(z)$，且$z_1$是任取的，那么$\phi(z)$全平面解析，即
[^7]:函数仅含有可去奇点时，可以通过解析延拓化为全纯函数。
...
$$
p(z)=\sum_{r=1}^{n}\frac{A_r}{z-z_r}+\varphi(z)\tag{1}
$$
其中$A_r$为$p(z)$在$z=z_r$处的留数[^3] ，$\varphi(z)$是全平面解析的函数，因为$z\rightarrow \infty(t\rightarrow0)$是方程的正则奇点，所以由式(15)可知，$\varphi(z\rightarrow\infty)=0$，由刘维尔定理可知[^4] ，$\varphi(z)=0$，所以
$$
...
$$
[^5]:无论是(16)还是(18) ，都代表着所有可能$p(z),q(z)$的形式。
...
$$
\begin{aligned}
u''+\left(\frac{A_1}{z}+\frac{A_2}{z-1}\right)u'+\left(\frac{C_1}{z}-\frac{C_1}{z-1}\right)u&=0\\
z(1-z)u''+\left[A_1-(A_1+A_2)z\right]u'+C_1u&=0\tag{24}
\end{aligned}
$$
我们对(24)式进行重参数化可以化为，
$$
z(1-z)u''+[\gamma-(\alpha+\beta+1)z]u'-\alpha\beta u=0\tag{25}
$$
式(25)被称为超几何方程，同时微分方程有三个正则奇点：$0,1,\infty$，方程的解为超几何级数：$F(\alpha,\beta,\gamma;z)$
...
|$zu''+(b-z)u'-au=0$|合流[^6]超几何方程|$a=c=0$,正则奇点$0$,非正则奇点$\infty$|
...
$$
P_3(z)u''+P_2(z)u'+P_1(z)u=0\tag{27}
$$
方程(24)被称为Heun方程。对其进行参数化为
$$
z(z-1)(z-a)u''+(\gamma+\delta z+\varepsilon z^2)u'+(az-q)u=0\tag{28}
$$
方程的奇点为$0,1,a,\infty$
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTc2MzUwMDAsLTE5ODg2NzAyMDAsLTcyNz
E4MzY3OSwtMjEwNTYwNjQxMCwtMTA2OTA0NDQxLC0xNjYyNDE1
Mzk3LDE3MDkxNTM5ODcsLTE4NDQ2MzMyMjQsLTE3MzU0MDg1MT
EsMTQ4NjgxNTczNCwtMTcxMDY2OTM2MSwyMDMwOTYxODY4LDQy
OTQ4MDQ1OSwzNzY1MzA4MDQsMTg5OTI3NjI5MiwtNjIwNjc4Nj
k0LC02MDcxMjg4MTksLTQ4ODA0ODA3LC03ODUxMzM2OTAsLTE1
MTQzODQyODNdfQ==
-->