...
## 富克斯(Fuchs)型方程
### 极点
我们先来回忆一下极点的相关知识，当函数$f(z)$在$z=z_0\in \Omega$处仅有一个$n$阶极点，那么有
$$
f(z)=\frac{h(z)}{(z-z_0)^n}\tag{16}
$$
其中$h(z)$为全纯函数[^8] 。我们将$h(z)$在$|z-z_0|<R$的邻域内展开，
$$
h(z)=\sum_{n=0}^{+\infty}h_n(z-z_0)^n\tag{17}
$$
[^]由于函数$f(z)$仅有一个$n$阶极点，这里的全纯函数可以解析延拓到整个复平面。

可以得到(16)式为
$$
\begin{aligned}
f(z)&=\frac{1}{z-z_0}\sum_{n=0}^{+\infty}h_n(z-z_0)^n\\
&=\frac{1}{z-z_0}\left[h_0+h_1(z-z_0)+h_2(z-z_0)^2+h_3(z-z_0)^3+\dots\right]\\
&=\frac{h_0}{z-z_0}+\left[h_1+h_2(z-z_0)+h_3(z-z_0)^2+\dots\right]\\
&=\frac{h_0}{z-z_0}+\sum_{n=0}^{+\infty}h_{n+1}(z-z_0)^n\tag{18}
\end{aligned}
$$
...
方程的奇点为$0,1,a,\infty$
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQ1Nzg5MzM3LC0xMDY5MDQ0NDEsLTE2Nj
I0MTUzOTcsMTcwOTE1Mzk4NywtMTg0NDYzMzIyNCwtMTczNTQw
ODUxMSwxNDg2ODE1NzM0LC0xNzEwNjY5MzYxLDIwMzA5NjE4Nj
gsNDI5NDgwNDU5LDM3NjUzMDgwNCwxODk5Mjc2MjkyLC02MjA2
Nzg2OTQsLTYwNzEyODgxOSwtNDg4MDQ4MDcsLTc4NTEzMzY5MC
wtMTUxNDM4NDI4MywxMDUzMjM3NTg3LDE5MTU3NTk4MSwxNjYz
MTIyNDcyXX0=
-->