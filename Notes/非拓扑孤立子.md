...
$$
\mathcal{L}=\partial_\mu\Phi^*\partial^{\mu}\Phi-V(\Phi\Phi^*)\tag{16}
$$
$U(1)$对称性给出的守恒流为
$$
...
$$
\begin{aligned}
T_{00}&=(\dot{\phi}_1+i\dot{\phi}_2)(\dot{\phi}_1-i\dot{\phi}_2)+(\partial_r\phi_1+i\partial_r\phi_2)(\partial_r\phi_1-i\partial_r\phi_2)+V(\phi_1^2+\phi_2^2)\\
&=\dot{\phi}_1^2+\dot{\phi}_2^2+(\partial_r\phi_1)^2+(\partial_r\phi_2)^2+V(\phi_1^2+\phi_2^2)
\end{aligned}
$$
所以系统的能量可以表示为
$$
\begin{aligned}
E&=\int T_{00}dx^3\\
&=\int \dot{\phi}_1^2+\dot{\phi}_2^2+(\partial_r\phi_1)^2+(\partial_r\phi_2)^2+V(\phi_1^2+\phi_2^2)d^3x
\end{aligned}
$$
我们期望找到在守恒荷$Q$固定下的最小的能量状态的场满足什么条件。所以我们需要求解一个条件极值问题：
$$
E-\omega N=\int \dot{\phi}_1^2+\dot{\phi}_2^2+(\partial_r\phi_1)^2+(\partial_r\phi_2)^2+V(\phi_1^2+\phi_2^2)d^3x-2\omega\int \phi_1\dot{\phi}_2-\phi_2\dot{\phi}_1 d^3x\tag{21}
$$
其中$\omega$为拉式乘子。可以得到，
$$
\begin{aligned}
...
$$
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1MzcwOTU3MjEsLTk3NDM2MzQyLC0xND
M5MjM3NDgsLTg0MjAxMTAxMyw2Mzg3MDU0NjYsLTEzNTUyODQ1
ODEsNTg4MTY1MTQ3LC0xMTQ0OTczOTIzLDEwMDg3NTM5OTUsLT
IxNDI1NzQ5NDcsLTE2NjM1MTIyNjQsLTIwMjAyMDM5NSwyMTc1
MDk4NDQsLTExODk2NTE5NjMsLTYyMTI3MTY2OCw0Mjc4MjI2Nz
IsMzgwNDA1MjE2LDE4NjE5MzkzMjgsMTM0NzA5MjA1NiwxMTI3
OTc5MDI4XX0=
-->