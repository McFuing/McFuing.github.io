...
## 诱导拓扑
镶嵌在$\mathbb{E}^3$的通常的面的拓扑是通过$3$维开球的交构成的，这个过程可以化为更为一般的情形：有一个拓扑空间$(S,T)$,且$X$是$S$的子集。我们可以在$X$上定义一个拓扑：将集合$X$与$T$中的开集相交作为$X$上的开集。这被称为**诱导拓扑**，记为$X\cap T$。这就可以产生一个新的tuo'p
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTM1NzI2MDU1MywtMTg0MDQ3Njg3MywxMD
