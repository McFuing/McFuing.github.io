...
>若集合 $B=\left\{  U_{\alpha}\right\}$ 是拓扑 $T$ 的基$\iff$对于任意的开集$V\in T$ 以及点 $p\in V$,都存在一个$U_{\alpha}\in B$,使得 $p \in U_{\alpha}\subset V$ 成立。$\iff$定义新的子集族$\widetilde{B} =\{\widetilde{U}_\alpha \ |}$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTcxNDE0NzYwLC0xODYzNzc4NTQ2LC0yOT
